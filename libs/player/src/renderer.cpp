...
void player::Renderer::render(SDL_Renderer *rend) {
    ...
    SDL_Rect dst;
    SDL_Rect src = get_head_rect(500, 350, &dst);
    ...
    if (get_arm_rect(500, 350, &src, &dst, &outer_arm_src, &outer_arm_dst))
        if (SDL_RenderCopy(rend, arm_tex, &src, &dst) < 0)
            throw error::Error(error::ErrorType::SDL_RENDERCOPY_ERROR,
                               SDL_GetError());
    if (get_leg_rect(500, 350, &src, &dst, &outer_leg_src, &outer_leg_dst))
        if (SDL_RenderCopy(rend, leg_tex, &src, &dst) < 0)
            throw error::Error(error::ErrorType::SDL_RENDERCOPY_ERROR,
                               SDL_GetError());
    src = get_torso_rect(500, 350, &dst);
